...
const isWithinDays = (session, days) => {
  const now = new Date();
  const sd = parseSessionDate(session);
  return (now - sd) / (1000 * 60 * 60 * 24) <= days;
};

const isToday = (session) => getDateStr(parseSessionDate(session)) === getDateStr(new Date());
...
const isThisWeek = (session) => {
  ...
  return parseSessionDate(session) >= monday && parseSessionDate(session) <= now;
};
              ...
              <div style={{ padding: "14px 14px 12px", borderBottom: `1px solid ${theme.border}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.displayName || "Usuario"}</div>
                    <div style={{ fontSize: 13, color: theme.textSec, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.email}</div>
                  </div>
                </div>
                <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: syncing ? "#f59e0b" : "#10b981" }}>{ICONS.cloud}<span>{syncing ? "Guardando..." : "Sincronizado"}</span></div>
              </div>
              <button onClick={() => { onLogout(); setOpen(false); }} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "12px 14px", margin: "4px 0 2px", background: "none", border: "none", borderRadius: 8, color: "#ef4444", fontSize: 15, cursor: "pointer", textAlign: "left" }}>{ICONS.logout} Cerrar sesión</button>
            </>
          ) : (
            <>
              <div style={{ padding: "12px 14px 8px" }}>
                <div style={{ fontSize: 15, fontWeight: 600 }}>Sin sesión</div>
                <div style={{ fontSize: 13, color: theme.textSec, marginTop: 3, lineHeight: 1.4 }}>Sincroniza entre dispositivos</div>
              </div>
              <button onClick={() => { onLogin(); setOpen(false); }} style={{ display: "flex", alignItems: "center", gap: 10, width: "calc(100% - 12px)", padding: "12px 14px", margin: "6px 6px 6px", background: darkMode ? "#1c1c1c" : "#f0f0f0", border: `1px solid ${theme.border}`, borderRadius: 10, color: theme.text, fontSize: 15, fontWeight: 600, cursor: "pointer" }}>{ICONS.google} Continuar con Google</button>
            </>
          )}
        </div>
      )}
...
function StatsView({ categories, theme, darkMode, onClose }) {
  ...
  const filteredTasks = filter === "all" ? allTasks : allTasks.filter((t) => t.categoryId === filter);

  const taskStats = filteredTasks.map((task) => {
    const fs = task.sessions.filter(filterSession);
    return { ...task, filteredSessions: fs, periodTime: fs.reduce((s, x) => s + x.duration, 0) };
  }).filter((t) => t.periodTime > 0 || t.sessions.length > 0);

  taskStats.sort((a, b) => b.periodTime - a.periodTime);

  const totalPeriodTime = taskStats.reduce((s, t) => s + t.periodTime, 0);

  const catStats = categories.map((cat) => {
    const time = allTasks.filter((t) => t.categoryId === cat.id).reduce((s, t) => s + t.sessions.filter(filterSession).reduce((a, x) => a + x.duration, 0), 0);
    return { ...cat, periodTime: time };
  ...
  const getDailyData = () => {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      let total = 0;
      filteredTasks.forEach((t) => t.sessions.forEach((s) => { if (getDateStr(parseSessionDate(s)) === ds) total += s.duration; }));
      data.push({ date: ds, label, total });
    }
    return data;
  };
    ...
    <div style={{ position: "fixed", inset: 0, backgroundColor: theme.bg, zIndex: 100, overflow: "auto", animation: "fadeIn 0.2s ease" }}>
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 16px" }}>
        ...
        <div style={{ display: "flex", gap: 6, padding: "8px 0 16px", overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
          <button onClick={() => setFilter("all")} style={{ padding: "6px 12px", borderRadius: 16, border: filter === "all" ? "none" : `1px solid ${theme.border}`, backgroundColor: filter === "all" ? (darkMode ? "#333" : "#ddd") : "transparent", color: filter === "all" ? theme.text : theme.textSec, fontSize: 13, cursor: "pointer", flexShrink: 0 }}>Todas</button>
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setFilter(cat.id)} style={{ padding: "6px 12px", borderRadius: 16, border: filter === cat.id ? "none" : `1px solid ${theme.border}`, backgroundColor: filter === cat.id ? `${cat.color}22` : "transparent", color: filter === cat.id ? cat.color : theme.textSec, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 5, flexShrink: 0, whiteSpace: "nowrap" }}>
              <div style={{ width: 8, height: 8, borderRadius: 2, backgroundColor: cat.color }} />{cat.name}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding: "16px 0", textAlign: "center", borderBottom: `1px solid ${theme.border}` }}>
          <div style={{ fontSize: 34, fontWeight: 700, letterSpacing: -1 }}>{formatTotalTimeLong(totalPeriodTime)}</div>
        ...
        {dailyData.length > 1 && (
          <div style={{ padding: "20px 0", borderBottom: `1px solid ${theme.border}` }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: dailyData.length > 14 ? 2 : 3, height: 100 }}>
              {dailyData.map((d, i) => (
                <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
                  <div style={{ fontSize: 9, color: theme.textSec, opacity: d.total > 0 ? 1 : 0 }}>{d.total > 0 ? formatTotalTime(d.total) : ""}</div>
                  <div style={{ width: "100%", height: Math.max(3, (d.total / maxDayTime) * 80), backgroundColor: d.total > 0 ? (filter !== "all" ? categories.find((c) => c.id === filter)?.color || theme.accent : theme.accent) : (darkMode ? "#1c1c1c" : "#eee"), borderRadius: 3, opacity: d.total > 0 ? 0.8 : 0.3, transition: "height 0.3s" }} />
                  {dailyData.length <= 7 && <div style={{ fontSize: 10, color: theme.textSec, whiteSpace: "nowrap", opacity: 0.7 }}>{d.label.split(" ")[0]}</div>}
                </div>
              ))}
            </div>
          </div>
            ...
            {catStats.map((cat) => (
              <div key={cat.id} style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div style={{ width: 10, height: 10, borderRadius: 3, backgroundColor: cat.color }} /><span style={{ fontSize: 15, fontWeight: 500 }}>{cat.name}</span></div>
                ...
                <div style={{ height: 6, backgroundColor: darkMode ? "#1c1c1c" : "#eee", borderRadius: 3, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${(cat.periodTime / maxCatTime) * 100}%`, backgroundColor: cat.color, borderRadius: 3, transition: "width 0.3s" }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ padding: "20px 0 100px" }}>
          ...
          {taskStats.length === 0 && <div style={{ textAlign: "center", padding: "40px 0", color: theme.textSec, fontSize: 14 }}>Sin datos para este periodo</div>}
          {taskStats.map((task) => (
            <div key={task.id} style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: theme.textSec, display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
                    ...
                    {task.categoryName} · {task.filteredSessions.length} ses.
                    {task.completed && <span style={{ color: "#10b981" }}> ✓</span>}
                  </div>
                </div>
              ...
              <div style={{ height: 5, backgroundColor: darkMode ? "#1c1c1c" : "#eee", borderRadius: 3, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${(task.periodTime / maxTime) * 100}%`, backgroundColor: task.categoryColor, borderRadius: 3, opacity: 0.7, transition: "width 0.3s" }} />
              </div>
            </div>
          ))}
        </div>
...
function ConfirmModal({ title, message, confirmLabel, confirmColor, onConfirm, onCancel, theme }) {
  return (
    <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.6)", backdropFilter: "blur(8px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, animation: "fadeIn 0.2s ease" }} onClick={onCancel}>
      <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: theme.card, border: `1px solid ${theme.border}`, borderRadius: 16, padding: 24, maxWidth: 340, width: "100%" }}>
...
export default function App() {
  ...
  const exportData = () => {
    const d = JSON.stringify(categories, null, 2);
  ...
  return (
    <div style={{ minHeight: "100dvh", backgroundColor: theme.bg, color: theme.text, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", "Inter", sans-serif', transition: "background-color 0.3s, color 0.3s", WebkitFontSmoothing: "antialiased" }}>
      ...
      {timerView && activeTaskData?.task && (
        <div style={{ position: "fixed", inset: 0, backgroundColor: darkMode ? "rgba(0,0,0,0.92)" : "rgba(255,255,255,0.95)", backdropFilter: "blur(20px)", zIndex: 100, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, animation: "fadeIn 0.25s ease" }}>
          <button onClick={() => setTimerView(null)} style={{ position: "absolute", top: 20, right: 20, background: "none", border: "none", color: theme.textSec, cursor: "pointer", padding: 10 }}>{ICONS.x}</button>
          ...
          <div style={{ fontSize: 20, fontWeight: 600, color: theme.text, marginBottom: 40, textAlign: "center", padding: "0 20px" }}>{activeTaskData.task.name}</div>
          <div style={{ fontSize: "min(64px, 13vw)", fontWeight: 200, fontVariantNumeric: "tabular-nums", color: activeTask === timerView ? theme.text : theme.textSec, marginBottom: 40, letterSpacing: 3, transition: "color 0.3s" }}>
          ...
          <div style={{ marginTop: 40, display: "flex", gap: 32, color: theme.textSec, fontSize: 14 }}>
            <div style={{ textAlign: "center" }}><div style={{ fontSize: 22, fontWeight: 600, color: theme.text, marginBottom: 4, opacity: 0.8 }}>{formatTotalTimeLong(activeTaskData.task.totalSeconds)}</div><div>Total</div></div>
            <div style={{ textAlign: "center" }}><div style={{ fontSize: 22, fontWeight: 600, color: theme.text, marginBottom: 4, opacity: 0.8 }}>{activeTaskData.task.sessions.length}</div><div>Sesiones</div></div>
          </div>
          {activeTaskData.task.sessions.length > 0 && (
            <div style={{ marginTop: 28, maxWidth: 300, width: "90%" }}>
              <div style={{ fontSize: 12, color: theme.textSec, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 8, opacity: 0.6 }}>Últimas sesiones</div>
              {activeTaskData.task.sessions.slice(-5).reverse().map((s, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: `1px solid ${theme.border}`, fontSize: 13, color: theme.textSec }}>
      ...
      {confirmModal && <ConfirmModal {...confirmModal} onCancel={() => setConfirmModal(null)} theme={theme} />}
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 16px" }}>
        ...
        <div style={{ padding: "20px 0 12px", borderBottom: `1px solid ${theme.border}` }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              <button onClick={() => setShowStats(true)} title="Estadísticas" style={{ background: "none", border: `1px solid ${theme.border}`, borderRadius: 10, color: theme.textSec, cursor: "pointer", width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{ICONS.chart}</button>
              <button onClick={() => setDarkMode(!darkMode)} style={{ background: "none", border: `1px solid ${theme.border}`, borderRadius: 10, color: theme.textSec, cursor: "pointer", width: 40, height: 40, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{darkMode ? ICONS.sun : ICONS.moon}</button>
            </div>
          </div>
          {/* Add category button - full width on second row */}
        ...
        {activeTask && (() => {
          ...
          if (!at) return null;
          return (
            <div onClick={() => setTimerView(activeTask)} style={{ margin: "14px 0 0", padding: "14px 16px", borderRadius: 14, background: `linear-gradient(135deg, ${ac.color}22, ${ac.color}08)`, border: `1px solid ${ac.color}33`, display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, flex: 1 }}>
                ...
                <div style={{ minWidth: 0 }}><div style={{ fontSize: 15, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{at.name}</div><div style={{ fontSize: 12, color: theme.textSec }}>{ac.name}</div></div>
              </div>
        ...
        {showNewCategory && (
          <div style={{ margin: "14px 0 0", padding: 16, borderRadius: 14, backgroundColor: theme.card, border: `1px solid ${theme.border}` }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={addCategory} style={{ padding: "0 18px", borderRadius: 10, border: "none", backgroundColor: theme.accent, color: darkMode ? "#000" : "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer" }}>Crear</button>
              <button onClick={() => { setShowNewCategory(false); setNewCatName(""); }} style={{ padding: "0 14px", borderRadius: 10, border: `1px solid ${theme.border}`, backgroundColor: "transparent", color: theme.textSec, fontSize: 16, cursor: "pointer" }}>✕</button>
            </div>
          </div>
        )}
                ...
                {expandedCats.has(cat.id) && (
                  <div>
                    {activeTasks.map((task) => (
                      <div key={task.id} onClick={() => setTimerView(task.id)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 12px", marginBottom: 5, borderRadius: 12, backgroundColor: activeTask === task.id ? `${cat.color}12` : theme.card, border: `1px solid ${activeTask === task.id ? `${cat.color}33` : theme.border}`, cursor: "pointer" }}>
                        <div style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
                          <div style={{ fontSize: 15, fontWeight: 500, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{task.name}</div>
                          <div style={{ fontSize: 13, color: theme.textSec, marginTop: 3, display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                            {ICONS.clock}
                            <span>{formatTotalTimeLong(task.totalSeconds + (task.isRunning ? task.currentSeconds : 0))}</span>
                            {task.sessions.length > 0 && <span>· {task.sessions.length} ses.</span>}
                          </div>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 3, flexShrink: 0 }}>
                          {activeTask === task.id && <span style={{ fontSize: 14, fontWeight: 300, fontVariantNumeric: "tabular-nums", color: cat.color, marginRight: 2 }}>{formatTime(task.currentSeconds)}</span>}
                          <button onClick={(e) => { e.stopPropagation(); toggleTimer(task.id); }} style={{ width: 38, height: 38, borderRadius: "50%", border: "none", backgroundColor: activeTask === task.id ? cat.color : theme.surface, color: activeTask === task.id ? "#fff" : theme.textSec, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>{activeTask === task.id ? ICONS.pause : ICONS.play}</button>
                          <button onClick={(e) => { e.stopPropagation(); setConfirmModal({ title: "¿Completar?", message: `"${task.name}" se moverá a completadas.`, confirmLabel: "Completar", confirmColor: "#10b981", onConfirm: () => completeTask(task.id) }); }} style={{ background: "none", border: "none", color: "#10b981", cursor: "pointer", padding: 5, opacity: 0.5 }}>{ICONS.check}</button>
                          <button onClick={(e) => { e.stopPropagation(); setConfirmModal({ title: "¿Resetear?", message: `Se borrará todo el tiempo de "${task.name}".`, confirmLabel: "Resetear", confirmColor: "#f59e0b", onConfirm: () => resetTask(task.id) }); }} style={{ background: "none", border: "none", color: theme.textSec, cursor: "pointer", padding: 5, opacity: 0.4 }}>{ICONS.reset}</button>
                          <button onClick={(e) => { e.stopPropagation(); setConfirmModal({ title: "¿Eliminar?", message: `Se eliminará "${task.name}" permanentemente.`, confirmLabel: "Eliminar", confirmColor: "#ef4444", onConfirm: () => deleteTask(task.id) }); }} style={{ background: "none", border: "none", color: theme.textSec, cursor: "pointer", padding: 5, opacity: 0.4 }}>{ICONS.trash}</button>
                        </div>
                      </div>
                    ))}
                    ...
                    {showNewTask === cat.id ? (
                      ...
                      <button onClick={() => { setShowNewTask(cat.id); setShowNewCategory(false); setNewTaskName(""); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 14px", margin: "5px 0", borderRadius: 10, border: `1px dashed ${theme.border}`, backgroundColor: "transparent", color: theme.textSec, fontSize: 14, cursor: "pointer", width: "100%" }}>{ICONS.plus} Añadir tarea</button>
                    )}

                    {completedTasks.length > 0 && (
                      <div style={{ marginTop: 8 }}>
                        <button onClick={() => setShowCompleted(showCompleted === cat.id ? false : cat.id)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 0", background: "none", border: "none", color: theme.textSec, fontSize: 14, cursor: "pointer" }}>
                        ...
                        {showCompleted === cat.id && completedTasks.map((task) => (
                          <div key={task.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 14px", marginBottom: 4, borderRadius: 12, backgroundColor: theme.card, border: `1px solid ${theme.border}`, opacity: 0.6 }}>
                            ...
                            <div style={{ display: "flex", gap: 4 }}>
                              <button onClick={() => uncompleteTask(task.id)} title="Reactivar" style={{ background: "none", border: "none", color: "#f59e0b", cursor: "pointer", padding: 6 }}>{ICONS.reset}</button>
                              <button onClick={() => setConfirmModal({ title: "¿Eliminar?", message: `Se eliminará "${task.name}".`, confirmLabel: "Eliminar", confirmColor: "#ef4444", onConfirm: () => deleteTask(task.id) })} style={{ background: "none", border: "none", color: theme.textSec, cursor: "pointer", padding: 6, opacity: 0.4 }}>{ICONS.trash}</button>
                            </div>
                          </div>
                        ))}
                      </div>
